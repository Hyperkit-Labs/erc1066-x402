...
export async function createServer() {
  ...
  if (process.env.DEPLOYMENT_REGISTRY_PATH) {
    ...
    chainConfigs = Object.entries(registry).map(([chainId, config]: [string, any]) => ({
    ...
    const validatorAddresses = JSON.parse(
      process.env.VALIDATOR_ADDRESSES || "{}"
    );
    const registryAddresses = JSON.parse(
      process.env.REGISTRY_ADDRESSES || "{}"
    );

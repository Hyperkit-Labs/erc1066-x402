import { PublicClient, createPublicClient, http, fallback } from "viem";
...
let chainlistCache: Map<number, ChainlistChain> | null = null;
const CACHE_TTL = 3600000; // 1 hour
let lastFetchTime = 0;

export async function fetchChainlist(): Promise<Map<number, ChainlistChain>> {
  const now = Date.now();
  ...
  try {
    ...
    const chains = (await response.json()) as ChainlistChain[];
...
export async function getRpcUrlsFromChainlist(
  chainId: number
): Promise<string[]> {
...
export function createRpcClientWithFallback(
  chainId: number,
  rpcUrls: string[]
): PublicClient {
  if (rpcUrls.length === 0) {
...
export async function getBestRpcUrl(
  chainId: number,
  fallbackUrl?: string
): Promise<string> {
  try {
